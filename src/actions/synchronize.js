...
export const updateAllData = (response, museum, deleted = {}) => async (dispatch) => {
    ...
    await Promise.all(
        data.objects.map(item => setObjects(item).then(object => {
            update_data.objects = updateItemOrPush(museum.objects, item);
        ...
        response.museums.categories.map(item => setCategories(item).then(category => {
            update_data.categories = updateItemOrPush(museum.categories, item);
    ...
    await setMuseums(museum, update_data)
    .then(updatedMuseum => {
