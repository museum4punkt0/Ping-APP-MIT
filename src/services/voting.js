...
export const getScore = (user, object, categories, settings, swipePenalty = 1) => {
    let score = 0;
    ...
    // decrease objects score if it has been swiped left
    if(user.votings) {
        const objectVoting = convertToArray(user.votings).find(item => item.object_id === object.sync_id)
        score -= swipePenalty * (objectVoting ? objectVoting.vote_count : 0)
    }
