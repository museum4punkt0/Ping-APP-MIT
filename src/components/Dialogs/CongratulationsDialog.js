...
class Dialog extends Component {
    ...
    render(){
        const {visible, onRequestClose, category, level} = this.props;
        const {move} = this.state;
          ...
          <Modal visible={visible} onRequestClose={onRequestClose} transparent>
            <View style={styles.main.dialogRootContainer}>
              <View style={styles.main.dialogContentContainer}>
                <Image source={Match} style={[styles.common.pacificoTitle, {fontSize:scale(40)}]}>{strings.congratulations}</Image>   
                <Text style={styles.main.dialogContentText}>{strings.formatString(strings.youHaveCompleted, category, level)}</Text> 
