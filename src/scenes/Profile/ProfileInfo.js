        ...
        .then(response => setUser(response.users)
          .then(async () => {
            await AsyncStorage.multiRemove([
              ...
              'firstCollection', 'tappingTip', 'allObjectsBelongTip', 'twoObjectsTip', 'oneObjectFilterTip', 'twoObjectsFilterTip', 'firstFilterTip', 'firstObjectInPlan'
            ]);
    ...
    return (
      <Scene label={strings.info} isFooterShow index={5} loading={loading}>
        ...
        <ScrollView style={{flex:1}}>
          ...
          <View style={{marginTop: 10}}>
            <Button onPress={() => this.handleResetUserButtonPress()} title={strings.resetDevice} containerStyle={styles.profileWithProps({plan}).profileButton} />  
            <TouchableOpacity onPress={()=>Toaster.showMessage(strings.resetAppExplanation, ToasterTypes.MESSAGE)}>
              <Text style={styles.profile.explanationText}>{strings.whatDoes}</Text>
            </TouchableOpacity>
          </View>
