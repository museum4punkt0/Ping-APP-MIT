...
import {convertToArray, getImage, planString, getStorageItem} from '../../config/helpers';
...
class DetectLocation extends Component {
  ...
  componentWillMount(){
    ...
    getStorageItem("firstObjectInPlan").then((value) =>
      this.setState({
        chats: chatsObj.reverse(),
        planDialog: newPlan && typeof value !== "string",
      })
    );
  }
  ...
  render() {
    ...
    return (
      <Scene label={strings.myPlan} isFooterShow index={3}>
        ...
        <Button onPress={() => Actions.TinderScene()} title={strings.addMoreObjects} containerStyle={{marginHorizontal:35, backgroundColor:colors.green}}/> 
