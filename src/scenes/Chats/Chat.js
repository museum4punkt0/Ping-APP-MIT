...
class Chats extends Component {
  ...
  async componentWillMount(){
    ...
    // Update user's location when he finds an object in the museum
    if (from === 'MAP') {
      if (object.positionX && object.positionY && object.section)
        await updateUser({
          ...user,
          positionX: parseFloat(object.positionX),
          positionY: parseFloat(object.positionY),
...
// eslint-disable-next-line react/destructuring-assignment
  async nextMessage(messageID, id = this.props.object.sync_id, isNext = false) {
    ...
    const { chat, speed, msgArray, messageInput } = this.state;
    ...
    */
    ...
    if (
      msgArray.length &&
      msgArray[msgArray.length - 1].type === 'Image' &&
      !object.onboarding &&
      !nextStep?.text?.includes('Collection')
    ) {
...
handleCameraFunc(){
  ...
  async handleFinishDialog(object){
    ...
    await this.updateChat({...chat, history: JSON.stringify(msgArray), finished: true});
    const chats = getChats();
    if(object.positionX && object.positionY && object.section) await updateUser({ ...user, chats });
  ...
  async handleAppStart(){
    ...
    await updateUser({
      ...
      avatar: this.imgPath,
      language_style,
      section: museums.sections.filter(
  ...
  render(){
    ...
    if(object.from) switch (object.from) {      
      ...
      return (
        ...
        >
          <SafeAreaView
            style={{ flex: 0, backgroundColor: colors.black }}
          />
          <SafeAreaView
            style={{ flex: 1, backgroundColor: isInputShow ? colors.dark : colors.black }}
          >
            <Scene
              ...
              headerStyle={{ paddingVertical: 10 }}
            >
              <ScrollView
                style={{ flex: 1 }}
              ...
              >
                {msgArray.map((message, i) => (
                  <MessageView
                    ...
                    key={i}
                    message={message}
                    avatar={object.cropped_avatar || object.avatar}
              ...
              {options && options.length > 0 && (
                <View>
                  <Options
                    options={options}
                    optionSelected={(key) => this.optionSelected(key)}
                  />
                </View>
              )}
              {isInputShow && (
                <View style={{ backgroundColor: colors.dark }}>
                  <Input
                    ...
                    onStateChanged={(messageInput) =>
                      this.setState({ messageInput })
                    }
                    handleSendMessageButtonPress={() =>
                      this.handleSendMessageButtonPress()
                    }
                  />
                </View>
              )}
              {isChooseAvatarShow && (
                <View>
                  <ChooseAvatar
                    settings={settings}
                    handleChooseAvatarPress={(avatar) =>
                      this.handleChooseAvatarPress(avatar)
                    }
                  />
                </View>
              )}
